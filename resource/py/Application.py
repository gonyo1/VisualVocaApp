...
class MainWindow(QtWidgets.QMainWindow, mp):
    ...
    def get_github_json(self):
        json_file = os.path.abspath(f"{__dir__}/src/github.json")
        ...
        if (self.sending_from_widget != None):
            ...
            if obj.currentRow() != -1:
                ...
                status = GetImages.get_images_from_word(self.word, self.JSON_DATA["ImageDownCount"], self.JSON_DATA,
                                                        self.file_type)
                file_types = tuple(f"{__dir__}/voca/img/{self.word}/{extention}" for extention in self.file_type)
                ...
                if len(self.pics) < self.JSON_DATA["ImageDownCount"]:
                    no_image = Qt.QPixmap(os.path.abspath(f"{__dir__}/src/img/NoImage.svg"))
    ...
    def start_player(self):
        ...
        def play_alarm_sound():
            audio_path = os.path.abspath(f"{__dir__}/src/sound/chime.wav")
